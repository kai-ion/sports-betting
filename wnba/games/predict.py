#!/usr/bin/env python3
"""
WNBA Game Predictor — moneyline and spread predictions.
Uses ESPN data for team stats and recent form.
"""
...
def get_wnba_games():
    """Get today's WNBA games with odds from ESPN."""
    try:
        resp = requests.get(
        ...
        for event in data.get("events", []):
            ...
            for comp in comps:
                ...
                if odds_list:
                    o = odds_list[0]
                    # Moneyline is nested under odds[0].moneyline.home/away.close.odds
                    ml = o.get("moneyline", {})
...
def get_team_stats():
    """Get WNBA team standings/stats from ESPN."""
    try:
        resp = requests.get(
        ...
        for group in data.get("children", []):
            for team_entry in group.get("standings", {}).get("entries", []):
                ...
                teams[abbr] = {
                    ...
                    "opp_ppg": float(stats.get("avgPointsAgainst", 80)),
...
def predict_game(home_abbr, away_abbr, team_stats, odds):
    ...
    # Scoring model
    # Home projected = avg of (home offense + what away defense allows) + home court
    home_off = home.get("ppg", 80)
    away_def = away.get("opp_ppg", 80)
    away_off = away.get("ppg", 80)
    home_def = home.get("opp_ppg", 80)

    home_court = 2.0  # WNBA home court ~2 pts

    home_projected = (home_off * 0.5 + away_def * 0.5) + home_court
    away_projected = (away_off * 0.5 + home_def * 0.5)

    # Win probability adjustment based on win %
    home_strength = home.get("win_pct", 0.5)
    away_strength = away.get("win_pct", 0.5)
    strength_adj = (home_strength - away_strength) * 5  # Up to ~5 pts swing
...
def main():
    print(f"=== WNBA Game Predictor — {datetime.now().strftime('%Y-%m-%d %H:%M')} ===\n")
